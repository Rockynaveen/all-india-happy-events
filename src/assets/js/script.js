          ...
          markers: [
            {
              ...
              icon: {
                image:
                  "https://wporganic.com/html/weddingdir/assets/images/pin.png",
                iconsize: [35, 48],
                iconanchor: [17, 48],
              },
            },
...
// Popular search tag click -> put value into input and optionally submit/search
document.querySelectorAll("#popularSearches .tag").forEach((btn) => {
  btn.addEventListener("click", function () {
    document.getElementById("searchInput").value = this.textContent;
    // optionally trigger search action here, e.g. submit form or call API
  });
});

const closeCityPopup = document.getElementById("closeCityPopup");
if (closeCityPopup) {
  closeCityPopup.onclick = function () {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ============================
       CITY POPUP (FIXED)
    ============================ */
  const openCityBtn = document.getElementById("openCityPopup");
  const cityPopup = document.getElementById("cityPopup");
  const closeCityBtn = document.getElementById("closeCityPopup");

  if (openCityBtn) {
    openCityBtn.addEventListener("click", (e) => {
      e.stopPropagation(); // prevent navbar collapse from swallowing click
      e.preventDefault();
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // if elements not found → exit silently (prevents JS crash)
  if (!carousel || !nextBtn || !prevBtn) return;

  function scrollDistance() {
    let card = carousel.querySelector(".pc-item");
...
// footer-gallery intialize popup
$(document).ready(function () {
...
// BLOG SWIPER
var projectswiper = new Swiper(".rr-project-active", {
  loop: true,
  speed: 800,
  autoplay: {
    delay: 2500,
...
document.querySelectorAll(".wishlist i").forEach(icon => {
    icon.addEventListener("click", function(e){
        ...
        this.style.color = this.classList.contains("fa-solid") ? "orange" :"black";
    });
});
